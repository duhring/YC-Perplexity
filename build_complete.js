...
// Read all frame files and convert to base64 data URIs
let frameDataJS = '';
for (let i = 1; i <= 16; i++) {
    try {
        const frameData = fs.readFileSync(`frame_${i}.b64`, 'utf8').trim();
        frameDataJS += `            ${i}: 'data:image/jpeg;base64,${frameData}',\n`;
    } catch (e) {
        console.log(`Warning: Could not read frame_${i}.b64`);
    }
}
...
<head>
    ...
    <title>Perplexity CEO Interview - Key Segments</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <header>
        <h1>🔍 Perplexity CEO Interview Explorer</h1>
        <p>Interactive exploration of Arvind Srinivas discussing AI, competition, and the future of search</p>
        <a href="https://youtu.be/2jOnoTEk-xA" target="_blank" class="watch-full-btn">
            🎬 Watch Full Video
        </a>
    </header>
    ...
    <footer>
        ...
        <p class="attribution">Created with transcript analysis and AI assistance • Includes extracted video frames</p>
    </footer>

    <script>
        // Embedded video frame data (base64 encoded)
        const frameData = {
${frameDataJS}        };
        ...
        function createSegmentCard(segment) {
            ...
            const hasFrame = frameData[segment.id];
            const imageContent = hasFrame 
                ? \`<img src="\${frameData[segment.id]}" alt="Video frame for \${segment.title}">\`
...
fs.writeFileSync('perplexity-interview-with-frames.html', htmlTemplate);
console.log('✅ Complete HTML file created: perplexity-interview-with-frames.html');
console.log('📁 File size: ' + Math.round(fs.statSync('perplexity-interview-with-frames.html').size / 1024) + 'KB');
EOF < /dev/null